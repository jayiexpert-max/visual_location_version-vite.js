...
export function DashboardPage() {
  ...
  const mqttQuery = useQuery({
    queryKey: ['dashboard', 'mqtt-health'],
    queryFn: healthService.getMqttHealth,
    refetchInterval: 60_000,
  });

  const raspiQuery = useQuery({
    queryKey: ['dashboard', 'raspi-health'],
    queryFn: healthService.getRaspiHealth,
    refetchInterval: 60_000,
  });
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid size={{ xs: 12, sm: 6, md: 4, lg: 2.4 }}>
          <StatusCard
            ...
            online={
              raspiQuery.data
                ? Number((raspiQuery.data as { onlineDevices?: number }).onlineDevices ?? 0) > 0
                : null
            }
            loading={raspiQuery.isLoading}
          />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 4, lg: 2.4 }}>
          <StatusCard
            title={t('pages:mqttStatus')}
            online={mqttQuery.data ? mqttQuery.data.connected : null}
            loading={mqttQuery.isLoading}
          />
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 4, lg: 2.4 }}>
          <StatusCard
